...
"""
最终版任务控制器（v18.2 - 最终修正版）：
- 修复了 'random' is not defined 的致命错误。
...
class TaskController:
    ...
    HOME_POSE = (0.1, 0.1, 0.0)

    NAVIGATION_TIMEOUT = 45.0
    MONITOR_RATE = 5.0
    TOTAL_MISSION_TIMEOUT = 600.0
    ...
    ROSBRIDGE_URL = "ws://192.168.250.32:9090"
    ...
    def __init__(self):
        ...
        rospy.loginfo("任务控制器 v18.2 (最终修正版) 已启动。")
    ...
    def navigate_home_simple(self):
        rospy.loginfo("开始精准返航...")
        goal = MoveBaseGoal(target_pose=PoseStamped(header=rospy.Header(frame_id="map", stamp=rospy.Time.now()),
                                                  pose=Pose(position=Point(self.HOME_POSE[0], self.HOME_POSE[1], 0),
                                                            orientation=Quaternion(*tf.transformations.quaternion_from_euler(0, 0, self.HOME_POSE[2])))))
        self.move_base_client.send_goal(goal)
        if self.move_base_client.wait_for_result(rospy.Duration(self.NAVIGATION_TIMEOUT)):
            if self.move_base_client.get_state() == actionlib.GoalStatus.SUCCEEDED:
                rospy.loginfo("已成功返回出发区。"); return True
        rospy.logerr("返航失败！"); self.stop_robot(); return False
    ...
    def peek_for_target(self):
        ...
        for direction in [1, -1]:
            ...
            while not rospy.is_shutdown() and (rospy.Time.now() - start_time).to_sec() < turn_duration_secs:
                self.cmd_vel_pub.publish(twist)
                if self.has_target_in_view(): self.stop_robot(); rospy.loginfo(f"观察中发现目标！"); return True
                rate.sleep()
    ...
    def report_mission_results(self):
        ...
        if self.found_item_name and self.found_target_location:
            ...
            rospy.sleep(0.1) # 短暂延时，避免同时发送导致问题
            ...
            if self.opportunistic_navigate(self.ROOM_SEARCH_AREAS[room_name], f"房间 {room_name}"):
                ...
                if self.peek_for_target():
                    if self.found_target_location is None:
                        ...
                        if self.current_detections: # 确保检测列表不为空
                            self.found_item_id = self.current_detections[0] # 取第一个检测到的物品
                            self.found_item_name = self.ITEM_ID_TO_NAME_MAP.get(self.found_item_id, "unknown_item")
                            rospy.loginfo(f"确认找到具体物品: ID={self.found_item_id}, 名称='{self.found_item_name}'")

            else:
                rospy.logwarn(f"未能进入房间 '{room_name}'，立即无缝前往下一个。")
        
        rospy.loginfo("===== 所有房间巡视完毕，开始返航 =====")
        if self.navigate_home_simple():
